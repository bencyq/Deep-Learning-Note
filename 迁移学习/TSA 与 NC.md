...
```python
mem_fea = torch.rand(31, 256).cuda()
...
classifier_loss = nn.CrossEntropyLoss()(y_t, pred)  # 伪标签的loss
total_loss = 0.1 * classifier_loss
```
...
```python
with torch.no_grad():
    softmax_t = nn.Softmax(dim=1)(y_t)
    _, pred_t = torch.max(softmax_t, 1)
    onehot_t = torch.eye(31)[pred_t].cuda()
    center_t = torch.mm(f_t.t(), onehot_t) / (onehot_t.sum(dim=0) + 1e-8)
mem_fea = 0.9 * mem_fea + 0.1 * center_t.t().clone()
```

## 修改了验证模块

```python
def validate(val_loader: DataLoader, model: ImageClassifier, args: argparse.Namespace, mem_fea) -> float:
    ...
    with torch.no_grad():
        for i, (images, target, _) in enumerate(val_loader):
            ...
            if start_test:
                ...
                all_f_t = torch.cat((all_f_t, f_t.float()), 0)
                all_label = torch.cat((all_label, target.float()), 0)
        mem_fea_norm = mem_fea / torch.norm(mem_fea, p=2, dim=1, keepdim=True)
        dis = torch.mm(all_f_t.detach(), mem_fea_norm.t())
        _, predict = torch.max(dis, 1)
        print(predict[:32])
        print(all_label[:32])
        print(predict.shape, all_label.shape)
        print(all_f_t.shape, mem_fea_norm.t().shape, dis.shape)
        print(torch.sum(predict[:32] == all_label[:32]).item())
...
def main(args: argparse.Namespace, config):
    ...
    for _, (images, label, index) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            ### 使用nc做伪标签 ###
            #             mem_fea_norm = mem_fea / torch.norm(mem_fea, p=2, dim=1, keepdim=True)
            #             dis = torch.mm(features.detach(), mem_fea_norm.t())
            #             _, pred = torch.max(dis, dim=1)
            #             pseudo_labels = pred
            ###
            pseudo_labels = predictions.argmax(1)  # 将最大值作为类别
            if flag:
                memory_target_features[index] = features[0].unsqueeze(0)  # ???1*256
    ...
    for epoch in range(args.epochs):
        ...
        if args.dset == "visda":
            ...
            acc1 = validate(val_loader, classifier, args, mem_fea)
...
def train(train_source_iter: ContinuousDataloader, train_target_iter: ContinuousDataloader, model: ImageClassifier,
          ...
          memory_source_features,
          memory_source_labels, memory_target_features, memory_target_labels,
          mem_fea):
    # switch to train mode
    model.train()
    max_iters = args.iters_per_epoch * args.epochs  # 200*40
    for i in range(args.iters_per_epoch):
        ...
        memory_target_labels[idx_target] = pred  # 使用nc伪标签

        #         print(pred.shape)
        #         print(y_t.argmax(1).shape)
        #         print(memory_target_labels.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dset', type=str, default='office', choices=['office', 'visda', 'office-home'],
                        help="The dataset used")
    parser.add_argument('--s_dset_path', type=str, default='/root/TSA/amazon.txt', help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='/root/TSA/webcam.txt', help="The target dataset path list")
    ...
    parser.add_argument('--iters-per-epoch', default=50, type=int, help='Number of iterations per epoch')
...
![无标题](https://raw.githubusercontent.com/bencyq/mixed/main/%E6%97%A0%E6%A0%87%E9%A2%98.png)